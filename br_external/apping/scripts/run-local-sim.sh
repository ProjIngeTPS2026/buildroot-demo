...
BASE_RUNTIME_CONFIGS=(
  "${APPING_BASE_ALPHA_CONFIG}"
  "${APPING_BASE_BETA_CONFIG}"
  "${APPING_BASE_GAMMA_CONFIG}"
  "${APPING_BASE_DELTA_CONFIG}"
  "${APPING_BASE_EPSILON_CONFIG}"
)
...
for config in "${BASE_RUNTIME_CONFIGS[@]}"; do
  base_name="$(basename "${config}" .json)"
  log_file="${APPING_LOG_DIR}/${base_name}.log"
  : > "${log_file}"
  "${APPING_BUILD_DIR}/fixed-base-service" --config "${config}" >>"${log_file}" 2>&1 &
  pids+=("$!")
done
...
echo "Base logs:"
for config in "${BASE_RUNTIME_CONFIGS[@]}"; do
  echo "  ${APPING_LOG_DIR}/$(basename "${config}" .json).log"
done
echo
